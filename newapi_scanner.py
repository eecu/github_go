...
# NewAPI 配置 (原 DMX)
NEWAPI_BASE_URL = "https://ai.burncloud.com"
NEWAPI_CHAT_URL = f"{NEWAPI_BASE_URL}/v1/chat/completions"

# 测试模型配置
TEST_MODELS = [
    "gpt-4.1-mini"  # 用户指定的测试模型
]
...
def test_model_access(api_key, model_name):
    """测试模型访问权限"""
    try:
        # NewAPI使用Authorization Bearer头部认证
        headers = {
        ...
        response = requests.post(NEWAPI_CHAT_URL, 
                               headers=headers, 
                               json=payload, 
                               timeout=15)
...
def save_live_key_immediately(key_info):
    ...
    with file_lock:
        ...
        if live_keys_filename is None:
            ...
            live_keys_filename = f'newapi_live_keys_{timestamp}.txt'
            simple_keys_filename = f'newapi_keys_simple_{timestamp}.txt'
            
            # 创建详细文件的头部
            with open(live_keys_filename, 'w', encoding='utf-8') as f:
                f.write("=== NewAPI 密钥扫描结果 (实时更新) ===\n")
...
def concurrent_search_and_validate():
    """并发搜索和验证NewAPI密钥"""
    print(f"🚀 开始并发搜索NewAPI密钥 (并发数: {MAX_WORKERS})")
    print("=" * 60)
    
    # 定义搜索查询
    search_queries = [
        '/sk-[A-Za-z0-9]{48,51}/ burncloud'
    ]
...
def save_results():
    ...
    # 如果没有实时保存（即没有发现存活密钥），创建传统的结果文件
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'newapi_live_keys_{timestamp}.txt'
        
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("=== NewAPI 密钥扫描结果 ===\n")
        ...
        # 保存简化版本（仅密钥）
        simple_filename = f'newapi_keys_simple_{timestamp}.txt'
...
def main():
    ...
    print("🔍 NewAPI 密钥并发扫描器 (Session Search Mode)")
    print("=" * 50)
    print(f"⚙️ 配置:")
